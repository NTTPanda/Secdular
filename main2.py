...
def update_display():
    ...
    for i, item in enumerate(upcoming):
        ...
        upcoming_tree.insert("", "end", values=item[:5], tags=(tag,))
...
style.configure("Treeview.Heading", font=("Arial", 11, "bold"), background="#444", foreground="Black")
style.map("Treeview", background=[("selected", "#0078D7")])  # Highlight selected row
style.layout("Treeview", [('Treeview.treearea', {'sticky': 'nswe'})])  # Remove border padding

# Row striping
style.configure("EvenRow.Treeview", background="#3A3A3A")  # Dark gray
style.configure("OddRow.Treeview", background="#2E2E2E")  # Slightly darker
...
# Apply row striping
upcoming_tree.tag_configure("evenrow", background="#3A3A3A")  # Light gray
upcoming_tree.tag_configure("oddrow", background="#2E2E2E")  # Slightly darker
